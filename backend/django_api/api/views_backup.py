...
from .serializers import (
    ...
    LoginSerializer, EmailVerificationSerializer, WaiverSerializer,
...
class PatientViewSet(viewsets.ModelViewSet):
    ...
    def create_my_profile(self, request):
        """Create or update a patient profile for the current user"""
        user = request.user
        ...
        # Also check for profiles without school year (created during signup)
        if not current_patient_profile:
        ...
        if current_patient_profile:
            ...
            if serializer.is_valid():
                patient = serializer.save(user=user, school_year=current_school_year)
    ...
    def update_my_profile(self, request):
        ...
        serializer = PatientProfileUpdateSerializer(current_patient_profile, data=request.data, partial=partial, context={'request': request})
    ...
    def autofill_data(self, request):
        """Get user information for autofill purposes"""
        user = request.user
        ...
        # Check if user has an existing profile for current school year
        existing_profile = None
        if current_school_year:
            existing_profile = user.patient_profiles.filter(school_year=current_school_year).first()
        ...
        # If there's an existing profile, include some data from it
        if existing_profile:
            autofill_data.update({
                'existing_profile_id': existing_profile.id,
                'existing_student_id': existing_profile.student_id,
                'existing_name': existing_profile.name,
                'profile_completion_status': {
                    'has_basic_info': bool(existing_profile.first_name and existing_profile.email),
                    'has_emergency_contact': bool(existing_profile.emergency_contact_surname and existing_profile.emergency_contact_first_name),
                    'has_health_history': bool(existing_profile.comorbid_illnesses or existing_profile.past_medical_history),
                    'has_family_history': bool(existing_profile.family_medical_history),
                }
            })
...
class AppointmentViewSet(viewsets.ModelViewSet):
    ...
    def view_medical_certificate(self, request, pk=None):
        """
        View the medical certificate for a completed appointment as PDF in browser
        """
        appointment = self.get_object()
        user = request.user
        ...
        if not (user.is_staff or user.user_type in ['staff', 'admin'] or 
                appointment.patient.user == user):
            raise PermissionDenied("You don't have permission to view this medical certificate.")
        
        # Check if appointment is completed
        if appointment.status != 'completed':
            return Response(
                {'error': 'Medical certificate is only available for completed appointments'}, 
                status=status.HTTP_400_BAD_REQUEST
            )
        
        try:
            # Find the medical document for this patient and school year
            ...
            # Serve the medical certificate file
            ...
            return Response(
                {'error': f'Failed to serve medical certificate: {str(e)}'}, 
                status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )
    ...
    def advise_for_consultation(self, request, pk=None):
        ...
        doc.reviewed_by = request.user
        doc.reviewed_at = timezone.now()
        doc.rejection_reason = reason  # Store consultation reason in rejection_reason field
        doc.save()
    ...
    @action(detail=True, methods=['post'])
    def cancel_consultation_advice(self, request, pk=None):
        """Cancel consultation advice and restore to pending (staff only)"""
        if not (request.user.is_staff or request.user.user_type in ['staff', 'admin']):
            raise PermissionDenied("Only staff can cancel consultation advice")
            
        doc = self.get_object()
        if doc.status != 'for_consultation':
            return Response({
                'detail': 'Document is not currently advised for consultation.'
            }, status=status.HTTP_400_BAD_REQUEST)
    ...
    @action(detail=False, methods=['get'])
    def for_consultation(self, request):
        """Get all documents advised for consultation (staff only)"""
        if not (request.user.is_staff or request.user.user_type in ['staff', 'admin']):
            raise PermissionDenied("Only staff can view consultation advice")
            
        queryset = self.get_queryset().filter(status='for_consultation')
    ...
    def get_patient_data(self, request):
        ...
        try:
            ...
            data = {
                ...
                'examined_by': f"{user.first_name} {user.last_name}".strip() if user.first_name or user.last_name else user.username,
