...
export default function PatientCertificateViewer() {
  ...
  const fetchDocumentAndStaff = async () => {
    try {
      setLoading(true);
      const docResponse = await medicalDocumentsAPI.getById(Number(id));
  ...
  // Format the full name properly - same logic as admin medical-certificate-viewer
  const getPatientFullName = () => {
    ...
    // Try using first_name/name fields from the serializer  
    const firstName = (medicalDoc as any).first_name || '';
    const middleName = (medicalDoc as any).middle_name || '';
    const lastName = (medicalDoc as any).name || '';
    ...
    // Fallback to patient_name or patient_display
    return (medicalDoc as any).patient_display || medicalDoc.patient_name || 'Patient';
  };
